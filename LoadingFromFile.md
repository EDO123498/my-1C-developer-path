...
|[.txt и .csv](#txt-and-csv)|[.xlsx](#xlsx)|[.dbf](#формат-txt-и-csv)|[.xml](#формат-txt-и-csv)|[.json](#формат-txt-и-csv)|
|-|-|-|-|-|
----
- [x] описание 1
...
### `xlsx`
----
>#### `☑ передача файла во временное хранилище`
```js
&НаКлиенте
...
Процедура ПрочитатьФайлНаСервереXLSX(АдресДанных)
	...
	Для СтрокаТаблицы = 2 по КоличествоСтрок Цикл
		...
		НоваяСтрока = Объект.СоставДокумента.Добавить();
		НоваяСтрока.Наименование	= ТабличныйДокумент.ПолучитьОбласть("R" + Формат(СтрокаТаблицы, "ЧГ=0") + "C" + 1).ТекущаяОбласть.Текст;
		НоваяСтрока.Статус		= ТабличныйДокумент.ПолучитьОбласть("R" + Формат(СтрокаТаблицы, "ЧГ=0") + "C" + 2).ТекущаяОбласть.Текст;
		НоваяСтрока.ИНН 		= ТабличныйДокумент.ПолучитьОбласть("R" + Формат(СтрокаТаблицы, "ЧГ=0") + "C" + 3).ТекущаяОбласть.Текст;
